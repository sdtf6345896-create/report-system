...
function checkLogin() {
    fetch('/api/auth/check', { credentials: 'include' })
        ...
        .then(data => {
            ...
            if (data.role !== 'ADMIN') { window.location.href = '/index.html'; return; }
            loadDepartments();
            loadReports();
        });
}
...
function switchTab(tab, btn) {
    ...
    if (tab === 'machine') { loadMachineFilters(); loadMachineList(); }
}
...
function deleteMachine(id) {
    ...
    fetch('/api/machine/' + id, { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); loadMachineFilters(); loadMachineList(); }
            else { alert('刪除失敗！'); }
        });
}
...
function loadReports() {
    ...
    fetch(url, { credentials: 'include' })
        ...
        .then(data => {
            ...
            tbody.innerHTML = '';
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;">查無報工紀錄</td></tr>';
                return;
            }
            const groupedByDate = {};
            data.forEach(item => {
            ...
            sortedDates.forEach(date => {
                if (tbody.children.length > 0) {
                    ...
                    spaceRow.innerHTML = '<td colspan="16" style="height:8px;background:transparent;border:none;"></td>';
                    tbody.appendChild(spaceRow);
                }
                const dateRow = document.createElement('tr');
                dateRow.className = 'date-header';
                dateRow.innerHTML = `<td colspan="16">${date.substring(0,4)}/${date.substring(4,6)}/${date.substring(6,8)}</td>`;
                ...
                headerRow.innerHTML = `
                    ...
                    <td>數量</td><td>報廢數量</td><td>備註</td><td>完批</td><td>操作</td>
                `;
                tbody.appendChild(headerRow);
                const groupedByDept = {};
                    ...
                    Object.keys(groupedByEmp).forEach(empNo => {
                        ...
                        empItems.forEach((item, index) => {
                            ...
                            html += `
                                ...
                                <td>${r.remarks || ''}</td>
                                <td style="text-align:center;">${r.isComplete ? '✔' : ''}</td>
...
function loadHoursStats() {
    ...
    if (type === 'employee') {
        ...
        if (empNo) params.push('employeeNo=' + empNo);
        if (params.length > 0) url += '?' + params.join('&');
        fetch(url, { credentials: 'include' }).then(res => res.json()).then(data => renderEmpStats(data));
    } else {
        ...
        fetch(url, { credentials: 'include' }).then(res => res.json()).then(data => renderMachineStats(data));
    }
}
...
function renderEmpStats(data) {
    ...
    tbody.innerHTML = '';
    if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">查無資料</td></tr>'; return; }
    const headerRow = document.createElement('tr');
    ...
    Object.keys(groupedByDept).forEach(deptName => {
        ...
        deptItems.forEach((item, index) => {
            const row = document.createElement('tr');
            let html = '';
            if (index === 0) html += `<td rowspan="${deptItems.length}" class="dept-cell">${deptName}</td>`;
            html += `
...
function renderMachineStats(data) {
    const tbody = document.getElementById('hoursBody');
    tbody.innerHTML = '';
    if (data.length === 0) { tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;">查無資料</td></tr>'; return; }
    const headerRow = document.createElement('tr');
    headerRow.className = 'hours-header';
    ...
    Object.keys(groupedByType).forEach(typeName => {
        ...
        typeItems.forEach((item, index) => {
            const row = document.createElement('tr');
            let html = '';
            if (index === 0) html += `<td rowspan="${typeItems.length}" class="dept-cell">${typeName}</td>`;
            html += `
...
function loadEditProcessList() {
    ...
    fetch('/api/workorder/' + order1 + '-' + order2)
        .then(res => res.json())
        .then(data => {
...
function onEditProcessChange() {
    ...
    if (!processNo) { document.getElementById('editProcessCode').value = ''; window._editAvailableMachines = []; return; }
        ...
        .then(data => {
            if (data.length > 0) {
                ...
                fetch('/api/machine/byProcess/' + data[0].processCode)
                    .then(res => res.json())
                    .then(machines => { window._editAvailableMachines = machines; onEditMachineQtyChange(); });
            } else {
                document.getElementById('editProcessCode').value = '';
                window._editAvailableMachines = [];
            }
...
function saveEdit() {
    ...
    if (!laborStart || !laborEnd || laborStart === '0' || laborEnd === '0') { alert('請填寫人時！'); return; }
    if (laborStart.length !== 4 || laborEnd.length !== 4) { alert('人時格式錯誤，請填寫4位數（例如：0800）！'); return; }
    if (qty > 0) {
        if (!machineTimeStart || !machineTimeEnd || machineTimeStart === '0' || machineTimeEnd === '0') { alert('請填寫機時！'); return; }
        if (machineTimeStart.length !== 4 || machineTimeEnd.length !== 4) { alert('機時格式錯誤，請填寫4位數（例如：0800）！'); return; }
        for (let i = 1; i <= qty; i++) {
    ...
    for (let i = 1; i <= qty; i++) {
        const mc = document.getElementById('editMachineCode' + i);
        if (mc && mc.value) machines.push({ machineCode: mc.value, machineTimeStart, machineTimeEnd });
    }
    ...
    fetch('/api/report/update/' + currentEditId, {
        ...
        if (res.ok) { alert('更新成功！'); closeModal(); loadReports(); }
        else { alert('更新失敗！'); }
    });
}
...
function deleteReport() {
    ...
    console.log('刪除 ID:', currentEditId);
    fetch('/api/report/delete/' + currentEditId, { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); closeModal(); loadReports(); }
            else { alert('刪除失敗！'); }
        });
}
...
function loadOrderList() {
    fetch('/api/workorder/list', { credentials: 'include' })
        ...
        .then(list => {
            ...
            const headerRow = document.createElement('tr');
            headerRow.className = 'sub-header';
            headerRow.innerHTML = '<td>製令號</td><td>品號</td><td>製令數量</td><td>完批進度</td><td>操作</td>';
            tbody.appendChild(headerRow);
            if (list.length === 0) {
                tbody.innerHTML += '<tr><td colspan="5" style="text-align:center;">查無製令</td></tr>';
                return;
            }
            list.forEach(w => {
                const row = document.createElement('tr');
                row.id = 'order-row-' + w.id;
                row.innerHTML = `
                    <td>${w.orderNo}</td>
                    <td>${w.productNo || ''}</td>
                    <td>${w.quantity || 0}</td>
                    <td id="progress-${w.id}">載入中...</td>
                    <td><button class="btn-delete" style="font-size:12px;padding:3px 8px;" onclick="deleteOrder(${w.id})">刪除</button></td>
                `;
                tbody.appendChild(row);
                fetch('/api/admin/workorder/' + encodeURIComponent(w.orderNo) + '/progress', { credentials: 'include' })
                    .then(res => res.json())
                    .then(progress => {
                        const cell = document.getElementById('progress-' + w.id);
                        if (cell) {
                            const progressText = w.isComplete ? '完批' : progress.completedProcesses + '/' + progress.totalProcesses;
                            cell.innerHTML = `<span style="cursor:pointer;color:#22d3ee;text-decoration:underline;"
                                onclick="openOrderDetail('${w.orderNo}', ${w.id}, ${progress.totalProcesses})">${progressText}</span>`;
                        }
                    });
            });
        });
...
function completeWorkOrder(id, orderNo, completedCount, totalProcesses) {
    if (completedCount < totalProcesses) {
        alert('尚有工序未完批！目前進度：' + completedCount + '/' + totalProcesses);
        return;
    }
    if (!confirm('確定要完批此製令嗎？')) return;
    fetch('/api/admin/workorder/' + id + '/complete', { method: 'PUT', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('製令完批成功！'); closeOrderDetail(); loadOrderList(); }
            else { alert('完批失敗！'); }
        });
}
...
function saveNewProduct() {
    ...
    for (let i = 1; i < table.rows.length; i++) {
        ...
        if (inputs[0].value && inputs[1].value && inputs[2].value)
            processes.push({ processNo: inputs[0].value.trim(), processCode: inputs[1].value.trim(), processName: inputs[2].value.trim() });
    }
    if (processes.length === 0) { alert('請至少新增一筆工序！'); return; }
    const imageFile = document.getElementById('addProductImage').files[0];
    const saveProcesses = () => {
        Promise.all(processes.map(p => fetch('/api/admin/process', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
            body: JSON.stringify({ productNo, ...p })
        }))).then(() => { closeAddProduct(); loadProductList(); loadProductNos(); openProductDetail(productNo); });
    };
    if (imageFile) {
        ...
        fetch('/api/admin/upload', { method: 'POST', credentials: 'include', body: formData }).then(() => saveProcesses());
    } else {
        saveProcesses();
    }
}
...
function closeAddOrder() { document.getElementById('addOrderModal').classList.remove('show'); }
...
function saveNewOrder() {
    ...
    fetch('/api/admin/workorder', {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ orderNo: order1 + '-' + order2, productNo, quantity: parseInt(document.getElementById('addOrderQuantity').value) || 0 })
    }).then(res => {
        if (res.ok) { alert('製令新增成功！'); closeAddOrder(); loadOrderList(); }
        else { alert('新增失敗，製令可能已存在！'); }
    });
}

function deleteOrder(id) {
    if (!confirm('確定要刪除此製令嗎？')) return;
    fetch('/api/admin/workorder/' + id, { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); loadOrderList(); }
            else { alert('刪除失敗！'); }
        });
}
...
function deleteProductImage() {
    ...
    fetch('/api/document/' + productNo).then(res => res.json()).then(docs => {
...
function openProductDetail(productNo) {
    ...
    fetch('/api/process/byProduct/' + productNo).then(res => res.json()).then(processes => {
        ...
        processes.forEach(p => {
            ...
            row.innerHTML = `
                ...
                <td id="sop_${p.processNo}">
                    <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                           onchange="uploadDoc('${productNo}', '${p.processNo}', 'SOP', this)" />
                </td>
                <td id="sip_${p.processNo}">
                    <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                           onchange="uploadDoc('${productNo}', '${p.processNo}', 'SIP', this)" />
                </td>
                <td>
                    <button class="btn-edit" style="font-size:12px;padding:3px 8px;margin-right:4px;"
                            onclick="openEditProcess('${productNo}', '${p.processNo}', '${p.processCode}', '${p.processName || ''}')">編輯</button>
                    <button class="btn-delete" style="font-size:12px;padding:3px 8px;"
                            onclick="deleteProcess('${productNo}', '${p.processNo}')">刪除</button>
                </td>
            `;
            fetch('/api/document/' + productNo + '/' + p.processNo).then(res => res.json()).then(docs => {
                docs.forEach(doc => {
                    ...
                    cell.innerHTML = isPdf
                        ? `<a href="/api/admin/file/${doc.filePath}" target="_blank"
                              style="color:green;font-size:11px;display:block;">📄 ${doc.filePath.split('/').pop()}</a>
                           <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                                  onchange="uploadDoc('${productNo}', '${p.processNo}', '${doc.docType}', this)" />`
                        : `<img src="/api/admin/file/${doc.filePath}"
                                style="width:60px;height:60px;object-fit:cover;cursor:pointer;border-radius:4px;"
                                onclick="window.open('/api/admin/file/${doc.filePath}','_blank')" />
                           <br>
                           <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                                  onchange="uploadDoc('${productNo}', '${p.processNo}', '${doc.docType}', this)" />`;
                });
...
function deleteProduct() {
    ...
    fetch('/api/admin/product/' + encodeURIComponent(productNo), { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); closeProductDetail(); loadProductList(); }
            else { alert('刪除失敗！'); }
        });
}
...
function uploadDoc(productNo, processNo, docType, input) {
    ...
    fetch('/api/admin/upload', { method: 'POST', credentials: 'include', body: formData })
        .then(res => { if (res.ok) return res.json(); throw new Error('上傳失敗'); })
        .then(data => {
            alert(docType + ' 上傳成功！');
            if (docType === 'IMG') {
                document.getElementById('detailProductImage').innerHTML = `
                ...
                const isPdf = data.fileName && data.fileName.toLowerCase().endsWith('.pdf');
                cell.innerHTML = isPdf
                    ? `<a href="/api/admin/file/${data.fileName}" target="_blank"
                          style="color:green;font-size:11px;display:block;">📄 ${data.fileName.split('/').pop()}</a>
                       <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                              onchange="uploadDoc('${productNo}', '${processNo}', '${docType}', this)" />`
                    : `<img src="/api/admin/file/${data.fileName}"
                            style="width:60px;height:60px;object-fit:cover;cursor:pointer;border-radius:4px;"
                            onclick="window.open('/api/admin/file/${data.fileName}','_blank')" />
                       <br>
                       <input type="file" accept="image/jpeg,image/jpg,image/png,.pdf" style="font-size:11px;"
                              onchange="uploadDoc('${productNo}', '${processNo}', '${docType}', this)" />`;
            }
        }).catch(err => { alert('上傳失敗：' + err.message); input.value = ''; });
}
...
function closeProcessModal() { document.getElementById('processModal').classList.remove('show'); }
...
function saveProcessModal() {
    ...
    if (window._processModalMode === 'add') {
        fetch('/api/admin/process', {
            ...
            if (res.ok) { alert('工序新增成功！'); closeProcessModal(); openProductDetail(productNo); }
            else { alert('新增失敗！'); }
        });
    } else {
        fetch('/api/admin/process/' + encodeURIComponent(productNo) + '/' + encodeURIComponent(window._processModalOldProcessNo), {
            ...
            if (res.ok) { alert('工序修改成功！'); closeProcessModal(); openProductDetail(productNo); }
            else { alert('修改失敗！'); }
        });
    }
}
...
function deleteProcess(productNo, processNo) {
    ...
    fetch('/api/admin/process/' + encodeURIComponent(productNo) + '/' + encodeURIComponent(processNo), { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); openProductDetail(productNo); }
            else { alert('刪除失敗！'); }
        });
}
        ...
        .then(emps => {
            ...
            emps.forEach(e => {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <button class="btn-edit" style="font-size:12px;padding:3px 8px;margin-right:4px;"
                                onclick="openEditEmployee(${JSON.stringify(e).replace(/"/g, '&quot;')})">細項</button>
                        <button class="btn-delete" style="font-size:12px;padding:3px 8px;"
                                onclick="deleteEmployee('${e.employeeNo}')">刪除</button>
                    </td>`;
                tbody.appendChild(row);
            });
        });
...
function openAddEmployee() {
    fetch('/api/admin/employees', { credentials: 'include' }).then(res => res.json()).then(emps => {
        let maxNo = 0;
        emps.forEach(e => { const no = parseInt(e.employeeNo); if (!isNaN(no) && no > maxNo) maxNo = no; });
        document.getElementById('newEmpNo').value = String(maxNo + 1).padStart(5, '0');
    });
    const select = document.getElementById('newEmpDept');
    select.innerHTML = '<option value="">--選擇部門--</option>';
    fetch('/api/admin/departments', { credentials: 'include' }).then(res => res.json()).then(depts => {
        depts.forEach(d => {
            const option = document.createElement('option');
            option.value = d.id;
            option.innerText = d.deptName;
...
function closeAddEmployee() { document.getElementById('addEmployeeModal').classList.remove('show'); }
...
function saveNewEmployee() {
    ...
    const saveEmployee = (photoPath) => {
        fetch('/api/admin/employee', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
            body: JSON.stringify({ employeeNo: empNo, employeeName: empName, deptId: parseInt(deptId), hireDate, gender, position, phone, idNumber, emergencyContact, emergencyPhone, emergencyRelation, photo: photoPath || '' })
        }).then(res => {
            if (res.ok) { alert('員工新增成功！'); closeAddEmployee(); loadEmployeeList(); loadDepartments(); }
            else { alert('新增失敗！'); }
        });
    };
    ...
    if (photoFile) {
        ...
        fetch('/api/admin/upload', { method: 'POST', credentials: 'include', body: formData })
            .then(res => res.json()).then(data => saveEmployee(data.path)).catch(() => saveEmployee(''));
    } else {
        saveEmployee('');
    }
}
...
function deleteEmployee(employeeNo) {
    ...
    fetch('/api/admin/employee/' + employeeNo, { method: 'DELETE', credentials: 'include' })
        .then(res => {
            if (res.ok) { alert('刪除成功！'); loadEmployeeList(); loadDepartments(); }
            else { alert('刪除失敗！'); }
        });
}
...
function openEditEmployee(e) {
    ...
    if (e.photo) {
        ...
        document.getElementById('editEmpPhotoPreview').innerHTML = `
            <img src="/api/admin/file/${fileName}" style="width:80px;height:80px;object-fit:cover;margin-bottom:5px;"><br>`;
    } else {
        document.getElementById('editEmpPhotoPreview').innerHTML = '';
    }
...
function closeEditEmployee() { document.getElementById('editEmployeeModal').classList.remove('show'); }
...
function saveEditEmployee() {
    ...
    const updateEmployee = (photoPath) => {
        fetch('/api/admin/employee/' + empNo, {
            method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
            body: JSON.stringify({ employeeName: empName, deptId: parseInt(deptId), hireDate, gender, position, phone, idNumber, emergencyContact, emergencyPhone, emergencyRelation, photo: photoPath })
        }).then(res => {
            if (res.ok) { alert('更新成功！'); closeEditEmployee(); loadEmployeeList(); }
            else { alert('更新失敗！'); }
        });
    };
    ...
    if (photoFile) {
        ...
        formData.append('docType', 'PHOTO');
        fetch('/api/admin/upload', { method: 'POST', credentials: 'include', body: formData })
            .then(res => res.json()).then(data => updateEmployee(data.path)).catch(() => updateEmployee(''));
    } else {
...
function openOrderDetail(orderNo, id, totalProcesses) {
    document.getElementById('orderDetailTitle').innerText = '製令 ' + orderNo + ' 細項';
    ...
    fetch('/api/admin/workorder/' + encodeURIComponent(orderNo) + '/detail', { credentials: 'include' })
        .then(res => res.json())
        .then(detail => {
            tbody.innerHTML = '';
            const headerRow = document.createElement('tr');
            headerRow.className = 'sub-header';
            headerRow.innerHTML = '<td>工序號</td><td>工序名稱</td><td>報工數量</td><td>完批</td>';
            tbody.appendChild(headerRow);

            if (detail.length === 0) {
                tbody.innerHTML += '<tr><td colspan="4" style="text-align:center;">查無工序</td></tr>';
                return;
            }

            let completedCount = 0;
            detail.forEach(d => {
                if (d.isComplete) completedCount++;
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${d.processNo}</td>
                    <td>${d.processName || ''}</td>
                    <td>${d.totalQty}</td>
                    <td style="text-align:center;">${d.isComplete ? '✔' : ''}</td>
                `;
                tbody.appendChild(row);
            });

            fetch('/api/admin/workorder/' + encodeURIComponent(orderNo) + '/progress', { credentials: 'include' })
                .then(res => res.json())
                .then(progress => {
                    const btnRow = document.createElement('tr');
                    if (progress.isComplete) {
                        btnRow.innerHTML = `
                            <td colspan="4" style="text-align:center;padding:10px;color:#22d3ee;">
                                ✔ 此製令已完批
                            </td>`;
                    } else {
                        btnRow.innerHTML = `
                            <td colspan="4" style="text-align:center;padding:10px;">
                                <button class="btn-save"
                                        onclick="completeWorkOrder(${id}, '${orderNo}', ${completedCount}, ${totalProcesses})"
                                        style="padding:6px 20px;">完批此製令</button>
                            </td>`;
                    }
